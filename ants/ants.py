...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.damage_is_doubled == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        present_bound = 0
        while place.is_hive == False and present_bound <= self.upper_bound:
            if place.bees != [] and present_bound >= self.lower_bound:
                present_bound += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            damage += self.damage
        # each object points to the original object
        # actual changes occur in the original list
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        random_bee = bees[random.randint(0, len(bees) - 1)]
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def new_action(gamestate):
            if target.left_turns == 0:
                Bee.action(target, gamestate)
                # target.action = Bee.action will cause an error
                # as if Bee.action needs another parameter self
                target.action = target.original_action
                return
            target.left_turns -= 1
            print("DEBUG:", target.left_turns)
            if gamestate.time % 2 == 0:
                Bee.action(target, gamestate)

        target.left_turns = self.slow_turns
        if hasattr(target, 'original_action') == False:
...
class LaserAnt(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    base_damage = 2
    distance_debuff = 0.25
    battery_debuff = 0.0625
    # END Problem EC 4
    ...
    def insects_in_front(self):
        ...
        while place != None:
            if place.ant != None:
                if place.ant.is_container == True and place.ant.ant_contained != self:
                    insects_to_damage[place.ant.ant_contained] = distance
                if place.ant != self:
                    insects_to_damage[place.ant] = distance
            for bee in place.bees:
                insects_to_damage[bee] = distance
            distance += 1
            place = place.entrance
        return insects_to_damage
        # END Problem EC 4

    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        distance_debuff_damage = distance * self.distance_debuff
        battery_debuff_damage = self.insects_shot * self.battery_debuff
        return max(0, self.base_damage - distance_debuff_damage - battery_debuff_damage)
        # END Problem EC 4
