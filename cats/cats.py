...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    select_limit = diff_function(typed_word, word_list[0], limit)
    select_word = word_list[0]
    for word in word_list:
        temp_limit = diff_function(typed_word, word, limit)
        if temp_limit < select_limit:
            select_limit = temp_limit
            select_word = word
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break # I'm not sure if break has been taught. Adding a variable can remove break. 
    if source != []:
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_time in tpp:
        timestamp = []
        for i in range(len(player_time) - 1):
            timestamp += [player_time[i + 1] - player_time[i]]
        times += [timestamp]
    # END PROBLEM 9
