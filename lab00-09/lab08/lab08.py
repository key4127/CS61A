...
def add_links(link1, link2):
    ...
    def copy_link(lnk):
        if type(lnk) != Link:
            return lnk
        return Link(copy_link(lnk.first), copy_link(lnk.rest))

    new_link1, new_link2 = copy_link(link1), copy_link(link2)
    new_link_node = new_link1
    while new_link_node.rest != Link.empty:
        new_link_node = new_link_node.rest
    new_link_node.rest = new_link2
    return new_link1
...
def multiply_lnks(lst_of_lnks):
    ...
    for lnk in lst_of_lnks:
        if lnk == Link.empty:
            return Link.empty
        product *= lnk.first
    lst_of_lnks_rests = multiply_lnks([lnk.rest for lnk in lst_of_lnks])
    return Link(product, lst_of_lnks_rests)
